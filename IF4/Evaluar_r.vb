...
'MessageBox.Show("Comenzando evaluacion de  consumo teorico")
        ...
        Dim res As Integer
        strDOB = InputBox("Fecha Inicial evaluacion (OT Producidas)", _
        "IF4 ", _
        "DD/MM/AA")
        ...
        If IsDate(strDOB) Then
            ...
            strDOB2 = InputBox("Fecha Final evaluacion (OT Producidas)", _
            "IF4 ", _
            "DD/MM/AA")
...
'MessageBox.Show("Rango de  validacion del " + CStr(strDOB) + " al " + CStr(strDOB2))
                ...
                Calculos_real.clean_dbayuda() ''-----// Se borra  el historico de muestras_t  para  mejorar  rendimiento y evitar  confusiones
                resumen_metods_real.clean_resumen()
 Calculos_real.clean_dbayuda_aux()
               'MessageBox.Show("Calculos  y resumen limpiados")
Calculos_real.carga_resultados_aux(strDOB, strDOB2)


'MessageBox.Show("ya  hay auxiliares")

                Calculos_real.carga_resultados(strDOB, strDOB2) ''-----Ciclo 1  conjuncion datos consumo-programa.carga  de datos  solo del periodo a  evaluar y primeros  calculos
                ' MessageBox.Show("calculos  teoricos  fase 1 (conjuncion datos  programa-consumo) y resumen datos genericos")



 Calculos_real.carga_resultados_second() ''-----Ciclo 2 de calculos
                ' MessageBox.Show("calculos  teoricos  fase 2 datos basados en muestras t  calculo 1 ")



Calculos_real.carga_resultados_third() ''-----Ciclo 3 de calculos
                ' MessageBox.Show("calculos  teoricos  fase 3 datos basados en muestras t  calculo 1 y 2 ")




Calculos_real.carga_resultados_fourth() ''-----Ciclo 4 y final  de calculos
...
resumen_metods_real.actualiza_resumen()
'MessageBox.Show("Actualizado resumen")

resumen_metods_real.actualiza_resumen2()
